#v5_test14-8-2_JQPARK_250811-1700
#v5 api
#retry code -> v5_test14-8-0_JQPARK_250721-1100
...
#               takeProfit=calc_result[18],
      ...
      time.sleep(1)
##############################################################################
def short_market_part(calc_result):
#   if(calc_result[0] == 0):
      print(session.place_order(   # 주문하기.
               category="linear",
               symbol=calc_result[0],   # 주문할 코인
               side='Sell',   # short주문
               orderType='Market', #'Limit',   # 시장가.
               qty=calc_result[2],   # 개수
               timeInForce="GTC",
...
#               takeProfit=calc_result[18],
      ...
      time.sleep(1)
##############################################################################
def short_limit_part(add_order):
#   if(calc_result[0] == 0):
      print(session.place_order(   # 주문하기.
               category="linear",
               symbol=add_order[0],   # 주문할 코인
...
#  if(open_order_condition == 0) or (open_order_condition != 0):
  ...
  max_price, min_price, now_price = max(h_list), min(l_list), c_list[0]
  upp_price, low_price = max_price, min_price
  ready_p = (now_price - min_price) / (max_price - min_price) * 100
  max_diff = max_price - min_price
  min_diff = c_list[0] * 0.5 / 5
  std_diff = max_diff * 0.3
  limit_diff = std_diff    
#-------------------------------------------------------------------------------
  if(ready_p > 70): std_price = max_price - std_diff
  elif(ready_p < 30): std_price = min_price + std_diff
  else:
...
#-------------------------------------------------------------------------------
  ...
  std = max(mx,mn)
  for m in range(std):
    if(h_list[m] != l_list[m]):
      if(upp_price <= h_list[m]) and (std_price >= l_list[m]):
        upper_calc = v_list[m] * ((upp_price - std_price) / (h_list[m] - l_list[m]))
      elif(upp_price <= h_list[m]) and (std_price <= l_list[m] <= upp_price):
        upper_calc = v_list[m] * ((upp_price - l_list[m]) / (h_list[m] - l_list[m]))
      elif(upp_price >= h_list[m]) and (std_price <= l_list[m] <= upp_price):
        upper_calc = v_list[m]
      elif(upp_price >= h_list[m]) and (h_list[m] >= std_price >= l_list[m]):
        upper_calc = v_list[m] * ((h_list[m] - c_list[m]) / (h_list[m] - l_list[m]))
      else: upper_calc = 0
    else:
      if(upp_price > h_list[m]) and (now_price < l_list[m]):
        upper_calc = v_list[m]
      else: upper_calc = 0
    upper_v.append(upper_calc) 

    if(h_list[m] != l_list[m]):
      if(std_price <= h_list[m]) and (low_price >= l_list[m]):
        lower_calc = v_list[m] * ((std_price - low_price) / (h_list[m] - l_list[m]))
      elif(std_price <= h_list[m]) and (low_price <= l_list[m] <= std_price):
        lower_calc = v_list[m] * ((std_price - l_list[m]) / (h_list[m] - l_list[m]))
      elif(std_price >= h_list[m]) and (low_price <= l_list[m]):
        lower_calc = v_list[m]
      elif(std_price >= h_list[m]) and (h_list[m] >= low_price >= l_list[m]):
        lower_calc = v_list[m] * ((h_list[m] - low_price) / (h_list[m] - l_list[m]))
      else: lower_calc = 0
    else:
      if(std_price > h_list[m]) and (low_price < l_list[m]):
        lower_calc = v_list[m]
  ...
  if(ready_p > 70) and (upper_per > 30): order_position = 1
  elif(ready_p < 30) and (lower_per > 30): order_position = 2
...
#-------------------------------------------------------------------------------
      order_type_list = []
      order_idx, order_tp, order_st = [0, 0, 0], [0, 0, 0], [0, 0, 0]
      get_open=session.get_open_orders(category="linear",symbol=sym_bol,orderFilter='Order')['result']['list']
      time.sleep(1)
      if(get_open != []):
        order_type = pd.DataFrame(get_open)['orderType']
        for num in range(len(order_type)): order_type_list.append(order_type[num])
      stop_order_list = [(stop_order["positionIdx"],stop_order["price"], stop_order["stopLoss"]) for stop_order in get_open if stop_order.get("orderType") == "Limit"]
      if(stop_order_list != []):
        for list in range(len(stop_order_list)):
          if(stop_order_list[list][0] == 1):
            order_idx[1], order_tp[1], order_st[1] = stop_order_list[list][0], float(stop_order_list[list][1]), float(stop_order_list[list][2])
          if(stop_order_list[list][0] == 2):
            order_idx[2], order_tp[2], order_st[2] = stop_order_list[list][0], float(stop_order_list[list][1]), float(stop_order_list[list][2])
#-------------------------------------------------------------------------------
#      if(get_open != []): keep_item[item_no] = 1
...
#-------------------------------------------------------------------------------
        if(get_open == []) and (long_qty == 0) and (float(calc_result[1]) != float(l_sym_lever)):
          session.set_leverage(category="linear", symbol=sym_bol, buyLeverage=calc_result[1], sellLeverage=s_sym_lever)
          time.sleep(1)
        if(get_open == []) and (short_qty == 0) and (float(calc_result[2]) != float(s_sym_lever)):
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
        if(value_v_list[item_no][0] in (1, 2, 3, 4)):
...
#            order_condition[item_no] = 'order_cancel'
        ...
        if(order_idx[1] == 1):
            if(order_tp[1] < float(l_order_price)):
              session.cancel_all_orders(category="linear", symbol=sym_bol,orderFilter='Order')
              time.sleep(1)
              order_condition[item_no] = 'L_order_cancel'
            elif(value_v_list[item_no][0] != 1):
              session.cancel_all_orders(category="linear", symbol=sym_bol,orderFilter='Order')
              time.sleep(1)
              order_condition[item_no] = 'L_order_cancel'

        if(order_idx[2] == 2):
            if(order_tp[2] > float(s_order_price)):
              session.cancel_all_orders(category="linear", symbol=sym_bol, orderFilter='Order')
              time.sleep(1)
...
#-------------------------------------------------------------------------------
              if(long_qty == 0) and (short_qty == 0) and ((invest_usdt * 2) < avail_usdt):
                if(float(min_value) < l_ex_value) and (value_v_list[item_no][0] == 1):
                  ...
                  order_condition[item_no] = 'Limit_order'
                  order_info[item_no] = [value_s_list[item_no], value_v_list[item_no]]
                  time.sleep(1)
...
#-------------------------------------------------------------------------------
              if(long_qty != 0) and (short_qty == 0) and (invest_usdt < avail_usdt):
                if(float(s_order_qty) != 0) and (float(min_value) < s_ex_value) and (value_v_list[item_no][0] == 2):
                    add_order = [sym_bol, 'Sell', s_order_qty, s_order_price, 2, s_st_price]
                    order_limit_part(add_order)
                    order_condition[item_no] = 'L_opened_S_order'
                    order_info[item_no] = [value_s_list[item_no], value_v_list[item_no]]
                    time.sleep(1)
#-------------------------------------------------------------------------------
              if(long_qty == 0) and (short_qty != 0) and (invest_usdt < avail_usdt):
                if(float(l_order_qty) != 0) and (float(min_value) < l_ex_value) and (value_v_list[item_no][0] == 1):
                    add_order = [sym_bol, 'Buy', l_order_qty, l_order_price, 1, l_st_price]
                    order_limit_part(add_order)
                    order_condition[item_no] = 'S_opened_L_order'
                    order_info[item_no] = [value_s_list[item_no], value_v_list[item_no]]
                    time.sleep(1)
#-------------------------------------------------------------------------------
        if(long_qty != 0):
